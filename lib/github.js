...
const { createPullRequest } = require('./pr-factory');
...
const createPullRequest = async () => {
    ...
    if (isBranch(githubOptions.ref)) {
        ...
        try {
            const pullRequest = createPullRequest(githubOptions);
            console.log(`Creating pull-request '${JSON.stringify(data)}' at repository '${githubOptions.repository}'`);

            const result = await client.post(`/repos/${githubOptions.repository}/pulls`, { body: JSON.stringify(pullRequest) });
