...
const extractGithubOptions = () => {
    return {
        ...
        prefix: env.extractVar('AUTOPR_TITLE_PREFIX')
    }
};
...
const createPullRequest = async () => {
    ...
    if (isBranch(githubOptions.ref)) {
        ...
        try {
            ...
            const pullRerquestResult = createPullRequestResult(JSON.parse(result.body));
            console.log(pullRerquestResult);
            await assignAuthorToPullRequest(pullRerquestResult.number, githubOptions);
            return pullRerquestResult;
        } catch (err) {
...
const assignAuthorToPullRequest = async (pullRequestId, githubOptions) => {
    const client = createClient(githubOptions.token);
    try {
        console.log(`Assigning '${githubOptions.actor}' to pull request '${pullRequestId}'`);
        const data = { assignees: [githubOptions.actor] };
        const result = await client.post(`/repos/${githubOptions.repository}/issues/${pullRequestId}/assignees`, { body: JSON.stringify(data) });
        console.log(`${result.statusCode}: ${result.statusMessage}`);
    } catch (err) {
        throw createHttpClientError(err);
    }
