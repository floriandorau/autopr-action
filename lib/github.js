...
const createBody = (featureBranch, actor) => {
...
const createData = ({ ref, base = 'master', actor = 'unknown' }) => {
    ...
    return {
        ...
        body: createBody(featureBranch, actor),
        head: featureBranch,
        base: base,
    };
};
...
const extractGithubOptions = () => {
    return {
        actor: extractVar("GITHUB_ACTOR"),
        token: extractVar("GITHUB_TOKEN"),
        repository: extractVar("GITHUB_REPOSITORY"),
        ref: extractVar("GITHUB_REF"),
        base: extractVar("GITHUB_BASE")
    }
};
...
const createPullRequest = async () => {
    ...
    if (isBranch(githubOptions.ref)) {
        ...
        try {
            ...
            return await client.post(`/repos/${githubOptions.repository}/pulls`, { body: JSON.stringify(data) });            
        } catch (err) {            
